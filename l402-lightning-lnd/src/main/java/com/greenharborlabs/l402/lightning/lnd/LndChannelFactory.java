...
/**
 * Factory for creating configured gRPC {@link ManagedChannel} instances
 * for connecting to an LND node.
 */
public final class LndChannelFactory {
     ...
     * @throws LndException          if TLS cert or macaroon files are missing/unreadable,
     *                               or if channel construction fails due to I/O errors
     * @throws IllegalStateException if {@code tlsCertPath} is null and {@code allowPlaintext} is false
     */
    ...
    private static ManagedChannel buildPlaintextChannel(LndConfig config) {
        ...
        var builder = ManagedChannelBuilder.forAddress(config.host(), config.port())
                .usePlaintext()
    ...
    private static ManagedChannel buildTlsChannel(LndConfig config) throws IOException {
        ...
        var builder = NettyChannelBuilder.forAddress(config.host(), config.port())
                .sslContext(sslContext)
                .keepAliveTime(config.keepAliveTimeSeconds(), TimeUnit.SECONDS)
                .keepAliveTimeout(config.keepAliveTimeoutSeconds(), TimeUnit.SECONDS)
                .keepAliveWithoutCalls(true)
