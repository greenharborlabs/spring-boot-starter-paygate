...
- **Paygate error responses are non-cacheable.** Every Paygate-generated 401, 402, malformed-credential, validation, and authentication failure includes `Cache-Control: no-store` and `X-Content-Type-Options: nosniff` while retaining its required authentication challenge semantics.
