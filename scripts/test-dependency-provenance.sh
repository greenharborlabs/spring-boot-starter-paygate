...
while IFS=$'\t' read -r case_name _; do
  ...
  case "$case_name" in
    ...
    source-coordinate-change) sed -i.bak '3s#https://repo1.maven.org/maven2/ch/qos/logback/logback-classic/1.5.34/logback-classic-1.5.34.jar#https://repo1.maven.org/maven2/ch/qos/logback/logback-core/1.5.34/logback-core-1.5.34.jar#' "$workspace/exceptions.tsv"; rm "$workspace/exceptions.tsv.bak" ;;
