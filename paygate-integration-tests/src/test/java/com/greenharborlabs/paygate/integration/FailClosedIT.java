...
/**
 * Integration test verifying the fail-closed security property: when the Lightning backend is
 * unreachable or unhealthy, the server must return 503 Service Unavailable instead of granting
 * access or returning 500.
 *
 * <p>Uses a controllable stub Lightning backend injected via a {@link TestConfiguration} that
 * overrides the auto-configured backend. The backend is stored in a static field because the
 * auto-configuration wraps it with caching/timeout decorators, making {@code @Autowired} injection
 * by concrete type impossible.
 */
@Tag("integration")
@SpringBootTest(
...
@TestPropertySource(
    properties = {
      ...
      "paygate.lnbits.api-key=fake-key-for-testing",
...
class FailClosedIT {
  ...
  class BackendUnhealthy {
    ...
    void healthEndpointStillWorks() throws Exception {
      try (var client = HttpClient.newHttpClient()) {
        var request =
            HttpRequest.newBuilder().uri(URI.create(baseUrl() + HEALTH_PATH)).GET().build();
        var response = client.send(request, HttpResponse.BodyHandlers.ofString());
  ...
  class InvoiceCreationFails {
    ...
    void setUp() {
      ...
      BACKEND.setCreateInvoiceException(new RuntimeException("Connection refused: localhost:1"));
    }
