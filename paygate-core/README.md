...
During macaroon verification, `MacaroonVerifier` first authenticates each caveat's exact UTF-8 `key=value` bytes in the HMAC chain. Only after that succeeds, it removes ASCII space (`U+0020`) and horizontal-tab (`U+0009`) edge padding from the key for verifier lookup. Thus a signed `\tservices\t=my_api:0` is evaluated by the registered `services` verifier, while its value, internal whitespace, Unicode whitespace, and line breaks remain unchanged. Register canonical, unpadded verifier keys only; padded registrations are rejected. If no verifier is found for a truly unregistered caveat key, that caveat is **skipped** -- verification continues without evaluating it. This supports L402 cross-service delegation for names this service does not understand. It does not make registered first-party boundary names optional: `route`, `method`, and the active `{serviceName}_capabilities` key are reserved and mandatory for `L402Validator`, so delegated credentials using those names are evaluated against this application's trusted boundary context. This differs from the original macaroons paper, which recommends failing closed on all unknown caveats.
