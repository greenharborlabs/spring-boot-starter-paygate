...
/**
 * A 32-byte payment preimage used in Lightning Network HTLC settlement. The corresponding payment
 * hash is SHA-256(preimage).
 *
 * <p>Implements {@link AutoCloseable} and {@link Destroyable} so that the sensitive preimage bytes
 * can be zeroized when no longer needed. After {@link #destroy()} or {@link #close()}, all
 * accessors throw {@link IllegalStateException}.
 */
public final class PaymentPreimage implements AutoCloseable, Destroyable {
  ...
  /**
   * Creates a new {@code PaymentPreimage} from the given 32-byte value. The input array is
   * defensively copied; the caller's array is NOT zeroized.
   *
  ...
  /**
   * Verifies that SHA-256(this preimage) equals the given payment hash, using constant-time
  ...
  /**
   * Zeroizes the internal preimage bytes using {@link KeyMaterial#zeroize(byte[])}. Idempotent --
  ...
  /**
   * Constant-time equality comparison using {@link MacaroonCrypto#constantTimeEquals}. Two
   * destroyed instances are never equal. Only locks {@code this}; reading {@code other.destroyed}
   * (volatile) and {@code other.data} without locking {@code other} is safe — at worst, a
   * concurrent destruction causes comparison against zeroed bytes, returning {@code false}.
   */
  @Override
