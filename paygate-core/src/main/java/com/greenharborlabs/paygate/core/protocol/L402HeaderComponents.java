 ...
 * Performs regex-based validation with DoS protection (macaroon portion limited to 8192
 * characters).
 */
public record L402HeaderComponents(String scheme, String macaroonBase64, String preimageHex) {
   ...
   * @throws L402Exception with {@link ErrorCode#MALFORMED_HEADER} if the header is null, empty, or
   *     malformed
   */
  public static L402HeaderComponents extractOrThrow(String authorizationHeader) {
    if (authorizationHeader == null || authorizationHeader.isEmpty()) {
      throw new L402Exception(
          ErrorCode.MALFORMED_HEADER, "Authorization header must not be null or empty", null);
    }
