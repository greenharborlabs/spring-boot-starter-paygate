...
/**
 * Static utility for path normalization and glob matching used by delegation caveat verifiers. Path
 * normalization delegates to {@link PathNormalizer} — the canonical source of truth. Zero external
...
public final class PathGlobMatcher {
  ...
  /**
   * Glob matching: splits pattern and path by / and walks segments pairwise. {@code *} as entire
   * segment matches exactly one segment. {@code *} within a segment is treated as literal. {@code
   * **} as the last segment matches zero or more remaining segments.
   */
  ...
  /**
   * Glob matching on already-normalized pattern and path. Callers must ensure both arguments have
   * been passed through {@link #normalizePath(String)} beforehand.
   */
  ...
  /**
   * Returns true if the proposed pattern is at least as narrow as (contained within) the existing
   * pattern. That is, every path matched by {@code proposedPattern} is also matched by {@code
   * existingPattern}.
   *
  ...
  /**
   * Containment check on already-normalized patterns. Callers must ensure both arguments have been
   * passed through {@link #normalizePath(String)} beforehand.
   */
  public static boolean isContainedInNormalized(
      String normalizedExisting, String normalizedProposed) {
