...
/**
 * Binary identifier embedded in an L402 macaroon. Layout: [version: 2-byte big-endian
 * uint16][paymentHash: 32 bytes][tokenId: 32 bytes] = 66 bytes.
 */
public record MacaroonIdentifier(int version, byte[] paymentHash, byte[] tokenId) {
  ...
  public MacaroonIdentifier {
    ...
    if (tokenId.length != HASH_BYTES) {
      throw new IllegalArgumentException(
          "tokenId must be exactly 32 bytes, got: " + tokenId.length);
    }
