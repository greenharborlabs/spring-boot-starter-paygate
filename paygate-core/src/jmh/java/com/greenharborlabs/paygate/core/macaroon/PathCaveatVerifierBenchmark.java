 ...
 *
 * <p>Each benchmark creates a caveat with N comma-separated glob patterns where the last pattern
 * matches the request path, forcing a full scan through all preceding non-matching patterns. This
 * measures worst-case verification time.
 */
...
public class PathCaveatVerifierBenchmark {
  ...
  /**
   * Builds a comma-separated pattern string with {@code count - 1} non-matching patterns followed
   * by the given matching pattern. Non-matching patterns use realistic but distinct path prefixes
