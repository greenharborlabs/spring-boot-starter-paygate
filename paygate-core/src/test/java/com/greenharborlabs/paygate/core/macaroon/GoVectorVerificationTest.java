 ...
 *
 * <ol>
 *   <li>The HMAC signature chain produces the expected signature
 *   <li>The V2 binary serialization matches the expected bytes
 *   <li>The base64 encoding matches the expected string
 * </ol>
 */
...
class GoVectorVerificationTest {
  ...
  void serializationMatches(VectorData vector) {
    List<Caveat> caveats =
        vector.caveatStrings().stream()
            ...
            .toList();
    Macaroon macaroon =
        new Macaroon(vector.identifier(), vector.location(), caveats, vector.expectedSignature());
  ...
  void deserializationRoundTrip(VectorData vector) {
    ...
    List<String> deserializedCaveatStrings =
        deserialized.caveats().stream().map(Caveat::toString).toList();
