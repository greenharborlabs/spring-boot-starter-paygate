...
class VirtualThreadConcurrencyTest {
    ...
    void hmacCorrectUnderVirtualThreadConcurrency() throws InterruptedException {
        ...
        for (int i = 0; i < threadCount; i++) {
            int index = i;
            Thread.ofVirtual().name("hmac-vt-", index).start(() -> {
                try {
                    ready.countDown();
    ...
    @Test
    @DisplayName("SHA-256 via PaymentPreimage.matchesHash is correct under 100 virtual thread concurrency")
    void sha256CorrectUnderVirtualThreadConcurrency() throws InterruptedException {
        ...
        for (int i = 0; i < threadCount; i++) {
            int index = i;
            Thread.ofVirtual().name("sha256-vt-", index).start(() -> {
                try {
                    ready.countDown();
                    go.await(5, TimeUnit.SECONDS);
                    ...
                    assertThat(matches)
                            .as("Preimage %d should match its hash", index)
                            .isTrue();
                } catch (Throwable t) {
                    errors.add(t);
                } finally {
    ...
    void hmacHighContentionSamePrototype() throws InterruptedException {
        ...
        for (int i = 0; i < threadCount; i++) {
            int index = i;
            Thread.ofVirtual().name("hmac-stress-vt-", index).start(() -> {
                try {
                    ready.countDown();
                    go.await(5, TimeUnit.SECONDS);
