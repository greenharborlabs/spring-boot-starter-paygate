...
/**
 * Extended round-trip tests for macaroon serialization/deserialization. Covers complex cases beyond
 * the Go-generated test vectors: 5 caveats, L402-specific identifier layout, with/without location.
 *
 * <p>Each test constructs a macaroon programmatically, computes the correct signature via the HMAC
 * chain, serializes to V2, deserializes back, and verifies all fields and the signature chain
 * independently.
 */
...
class ExtendedMacaroonRoundTripTest {
  ...
  /**
   * Verifies that a macaroon round-trips through serialize/deserialize and that the signature chain
  ...
  private static void assertRoundTrip(
    ...
    assertThat(deserialized.caveats()).as("caveats").containsExactlyElementsOf(caveats);
    ...
    assertThat(reserialized).as("re-serialized bytes").isEqualTo(serialized);

    // Base64 round-trip
    String base64 = Base64.getEncoder().encodeToString(serialized);
    byte[] fromBase64 = Base64.getDecoder().decode(base64);
    assertThat(fromBase64).as("base64 decode").isEqualTo(serialized);
  ...
  void fiveCaveatsRoundTrip() {
    byte[] rootKey =
        HEX.parseHex("0505050505050505050505050505050505050505050505050505050505050505");
    byte[] paymentHash =
        HEX.parseHex("a1a2a3a4a5a6a7a8a9a0b1b2b3b4b5b6b7b8b9b0c1c2c3c4c5c6c7c8c9c0d1d2");
    byte[] tokenId =
        HEX.parseHex("e1e2e3e4e5e6e7e8e9e0f1f2f3f4f5f6f7f8f9f0010203040506070809101112");
  ...
  void fiveCaveatsWithLocationRoundTrip() {
    byte[] rootKey =
        HEX.parseHex("0606060606060606060606060606060606060606060606060606060606060606");
    byte[] paymentHash =
        HEX.parseHex("1111111111111111111111111111111111111111111111111111111111111111");
    byte[] tokenId =
        HEX.parseHex("2222222222222222222222222222222222222222222222222222222222222222");
    byte[] identifier = buildIdentifier(0, paymentHash, tokenId);
  ...
  void l402TypicalCaveatsRoundTrip() {
    ...
    byte[] rootKey =
        HEX.parseHex("0707070707070707070707070707070707070707070707070707070707070707");
    byte[] paymentHash =
        HEX.parseHex("deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef");
    byte[] tokenId =
        HEX.parseHex("cafebabecafebabecafebabecafebabecafebabecafebabecafebabecafebabe");
    byte[] identifier = buildIdentifier(0, paymentHash, tokenId);
  ...
  void l402NoCaveatsNoLocationRoundTrip() {
    ...
    byte[] rootKey =
        HEX.parseHex("0808080808080808080808080808080808080808080808080808080808080808");
    byte[] paymentHash =
        HEX.parseHex("abcdef01abcdef01abcdef01abcdef01abcdef01abcdef01abcdef01abcdef01");
    byte[] tokenId =
        HEX.parseHex("fedcba98fedcba98fedcba98fedcba98fedcba98fedcba98fedcba98fedcba98");
    byte[] identifier = buildIdentifier(0, paymentHash, tokenId);
  ...
  void identifierVersionPreserved() {
    ...
    byte[] rootKey =
        HEX.parseHex("0909090909090909090909090909090909090909090909090909090909090909");
    byte[] paymentHash =
        HEX.parseHex("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
    byte[] tokenId =
        HEX.parseHex("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb");
    ...
    List<Caveat> caveats = List.of(new Caveat("services", "test_api:0"));
  ...
  void caveatWithSpecialCharactersRoundTrip() {
    byte[] rootKey =
        HEX.parseHex("0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a");
    byte[] paymentHash =
        HEX.parseHex("ff00ff00ff00ff00ff00ff00ff00ff00ff00ff00ff00ff00ff00ff00ff00ff00");
    byte[] tokenId =
        HEX.parseHex("00ff00ff00ff00ff00ff00ff00ff00ff00ff00ff00ff00ff00ff00ff00ff00ff");
    byte[] identifier = buildIdentifier(0, paymentHash, tokenId);
