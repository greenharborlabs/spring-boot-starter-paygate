...
class PathNormalizerTest {
  ...
  class Normalize {
    ...
    void happyPath() {
      assertThat(PathNormalizer.normalize("/api/products/123")).isEqualTo("/api/products/123");
    }

    @Test
    @DisplayName("strips query string")
    void queryStrip() {
      assertThat(PathNormalizer.normalize("/api/products?foo=bar")).isEqualTo("/api/products");
    }

    @Test
    @DisplayName("iterative decode resolves double-encoded traversal")
    void iterativeDecode() {
      assertThat(PathNormalizer.normalize("/api/%252e%252e/secret")).isEqualTo("/secret");
    }
    ...
    void reservedSlash() {
      assertThat(PathNormalizer.normalize("/api/v1%2Fbypass")).isEqualTo("/api/v1%2Fbypass");
    }

    @Test
    @DisplayName("reserved delimiter %3F preserved")
    void reservedQuery() {
      assertThat(PathNormalizer.normalize("/api/a%3Fb")).isEqualTo("/api/a%3Fb");
    }

    @Test
    @DisplayName("reserved delimiter %23 preserved")
    void reservedHash() {
      assertThat(PathNormalizer.normalize("/api/a%23b")).isEqualTo("/api/a%23b");
    }

    @Test
    @DisplayName("reserved delimiter %3A preserved")
    void reservedColon() {
      assertThat(PathNormalizer.normalize("/api/a%3Ab")).isEqualTo("/api/a%3Ab");
    }
    ...
    void hexUppercased() {
      assertThat(PathNormalizer.normalize("/api/a%2fb")).isEqualTo("/api/a%2Fb");
    }

    @Test
    @DisplayName("collapses consecutive slashes")
    void slashCollapse() {
      assertThat(PathNormalizer.normalize("/api///products")).isEqualTo("/api/products");
    }
    ...
    void plusLiteral() {
      assertThat(PathNormalizer.normalize("/api/a+b")).isEqualTo("/api/a+b");
    }
    ...
    void parentDotSegment() {
      assertThat(PathNormalizer.normalize("/api/../products")).isEqualTo("/products");
    }

    @Test
    @DisplayName("percent-encoded dot resolved")
    void percentEncodedDot() {
      assertThat(PathNormalizer.normalize("/api/%2e/products")).isEqualTo("/api/products");
    }
    ...
    void fiveLayerEncodedDotSegmentsResolved() {
      // 5 layers: %25252525252e%25252525252e -> after 4 passes: %2e%2e -> cleanup pass: .. ->
      // resolved
    ...
    void traversalAboveRootClamped() {
      assertThat(PathNormalizer.normalize("/../../../api")).isEqualTo("/api");
    }
  }
  ...
  class PercentDecode {
    ...
    void invalidHex() {
      assertThat(PathNormalizer.percentDecode("%ZZ")).isEqualTo("%ZZ");
    }

    @Test
    @DisplayName("incomplete percent at end passes through unchanged")
    void incompletePercent() {
      assertThat(PathNormalizer.percentDecode("abc%")).isEqualTo("abc%");
    }
  }
}
