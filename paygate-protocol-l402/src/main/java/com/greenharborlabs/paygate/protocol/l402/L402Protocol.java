...
/**
 * L402 protocol adapter that implements the protocol-agnostic {@link PaymentProtocol}
 * interface by delegating to the existing paygate-core L402 infrastructure.
 */
public class L402Protocol implements PaymentProtocol {
    ...
    public PaymentCredential parseCredential(String authorizationHeader)
        ...
        try {
            ...
            return new PaymentCredential(
                    ...
                    new L402Metadata(
                            credential.macaroon(),
                            credential.additionalMacaroons(),
                            authorizationHeader
                    )
            );
    ...
    public ChallengeResponse formatChallenge(ChallengeContext context) {
        ...
        MacaroonIdentifier identifier = new MacaroonIdentifier(
                MACAROON_IDENTIFIER_VERSION, context.paymentHash(), tokenIdBytes);
        ...
        caveats.add(new Caveat(serviceName + "_valid_until",
                String.valueOf(validUntil.getEpochSecond())));

        Macaroon macaroon = MacaroonMinter.mint(
                context.rootKeyBytes(), identifier, null, caveats);
    ...
    /**
     * Maps an L402 core {@link ErrorCode} to the protocol-agnostic
     * {@link PaymentValidationException.ErrorCode}.
     */
    private static PaymentValidationException mapL402Exception(L402Exception e) {
        PaymentValidationException.ErrorCode mapped = switch (e.getErrorCode()) {
     ...
     * HTTP header injection. Rejects C0 control characters (0x00-0x1F), DEL (0x7F),
     * and double-quote.
     */
