...
/**
 * A {@link MeterFilter} that caps the cardinality of the {@code endpoint} tag
 * on L402 meters. When the configured maximum is reached, new endpoint values
 * are replaced with the overflow sentinel (default: {@code "_other"}).
 */
public class L402MeterFilter implements MeterFilter {
    ...
    public L402MeterFilter(int maxEndpointCardinality, String overflowTagValue) {
    ...
    private Meter.Id replaceEndpointTag(Meter.Id id, String newValue) {
        var newTags = id.getTags().stream()
                .map(tag -> tag.getKey().equals(ENDPOINT_TAG)
                        ? Tag.of(ENDPOINT_TAG, newValue)
                        : tag)
                .toList();
