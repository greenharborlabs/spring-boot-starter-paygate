 ...
 * </ul>
 */
final class L402SecurityModeResolver {

    static final String PROPERTY_NAME = "l402.security-mode";
    ...
    private L402SecurityModeResolver() {
    }
    ...
    static void validate(String configured) {
        if (!VALID_MODES.contains(configured)) {
            throw new IllegalStateException(
                    "Invalid l402.security-mode value: '" + configured
                            + "'. Valid values: auto, servlet, spring-security");
        }
        if (MODE_SPRING_SECURITY.equals(configured) && !isSpringSecurityPresent()) {
            throw new IllegalStateException(
                    "l402.security-mode=spring-security but Spring Security is not on the classpath");
        }
    }
    ...
    static boolean isSpringSecurityPresent() {
        try {
            Class.forName(ENABLE_WEB_SECURITY_CLASS, false,
                    L402SecurityModeResolver.class.getClassLoader());
            return true;
