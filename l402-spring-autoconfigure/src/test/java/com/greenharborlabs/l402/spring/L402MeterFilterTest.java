...
@DisplayName("L402MeterFilter — endpoint tag cardinality capping")
class L402MeterFilterTest {

    private static final int DEFAULT_CAP = 100;
    private static final String DEFAULT_OVERFLOW = "_other";

    private L402MeterFilter filter;

    @BeforeEach
    void setUp() {
        filter = new L402MeterFilter(DEFAULT_CAP, DEFAULT_OVERFLOW);
    }
    ...
    void cardinalityCapBehavior() {
        ...
        for (int i = 0; i < DEFAULT_CAP; i++) {
            Meter.Id input = meterId("l402.requests", "endpoint", "/api/endpoint-" + i);
        ...
        Meter.Id overflow = meterId("l402.requests", "endpoint", "/api/endpoint-100");
    ...
    void noEndpointTagPassesThrough() {
        Meter.Id input = meterId("l402.requests", "result", "challenged");
        Meter.Id result = filter.map(input);
    ...
    void alreadySeenEndpointPassesThrough() {
        ...
        Meter.Id first = meterId("l402.requests", "endpoint", endpoint);
        Meter.Id second = meterId("l402.invoices.created", "endpoint", endpoint);
    ...
    void customOverflowTagValue() {
        ...
        var customFilter = new L402MeterFilter(2, customOverflow);

        // Fill the cap
        customFilter.map(meterId("l402.requests", "endpoint", "/a"));
        customFilter.map(meterId("l402.requests", "endpoint", "/b"));

        // Third should use custom overflow
        Meter.Id result = customFilter.map(meterId("l402.requests", "endpoint", "/c"));
