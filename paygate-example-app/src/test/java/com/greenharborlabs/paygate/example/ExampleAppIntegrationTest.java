 ...
 *
 * <p>Starts the full application context and exercises the complete L402 flow: unauthenticated
 * challenge, credential presentation, and access grant. Uses {@code paygate.root-key-store=memory}
 * so the test can mint valid macaroons against the same {@link InMemoryRootKeyStore} used by the
 * filter.
 */
...
class ExampleAppIntegrationTest {
  ...
  class AnalyzeEndpointNoAuth {
    ...
    void returns402WithDynamicPricing() throws Exception {
      ...
      mockMvc
          .perform(
              post("/api/v1/analyze").contentType(MediaType.APPLICATION_JSON).content(shortBody))
          .andExpect(status().isPaymentRequired())
          .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
          .andExpect(jsonPath("$.code", is(402)))
