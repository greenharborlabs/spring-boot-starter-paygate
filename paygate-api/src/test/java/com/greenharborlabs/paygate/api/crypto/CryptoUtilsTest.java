...
class CryptoUtilsTest {
  ...
  class ConstantTimeEquals {
    ...
    void identicalArrays() {
      assertThat(CryptoUtils.constantTimeEquals(new byte[] {1, 2, 3}, new byte[] {1, 2, 3}))
          .isTrue();
    }
    ...
    void differentValues() {
      assertThat(CryptoUtils.constantTimeEquals(new byte[] {1, 2, 3}, new byte[] {1, 2, 4}))
          .isFalse();
    }
    ...
    void emptyArrays() {
      assertThat(CryptoUtils.constantTimeEquals(new byte[0], new byte[0])).isTrue();
    }

    @Test
    @DisplayName("different-length arrays return false")
    void differentLengthArrays() {
      assertThat(CryptoUtils.constantTimeEquals(new byte[32], new byte[16])).isFalse();
    }
  }
