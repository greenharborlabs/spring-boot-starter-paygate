 ...
 *
 * <p>Implementations are registered as Spring beans and injected into the security
 * filter as a {@code List<PaymentProtocol>}. Each implementation handles a single
 * authentication scheme (e.g., L402, Payment).
 */
public interface PaymentProtocol {
     ...
     *
     * <p>The scheme is used to match incoming {@code Authorization} headers and to
     * format outgoing {@code WWW-Authenticate} challenges (e.g., {@code "L402"},
     * {@code "Payment"}).
     *
    ...
    /**
     * Returns whether this protocol can handle the given {@code Authorization}
     * header value.
     *
     * <p>Implementations should perform a fast, prefix-based check and must not
     ...
     */
    PaymentCredential parseCredential(String authorizationHeader)
            throws PaymentValidationException;

    /**
     * Formats a challenge context into a {@code WWW-Authenticate} header value and
     * accompanying response body data.
     *
     ...
     *
     * <p>Implementations should perform all protocol-specific verification (e.g.,
     * macaroon signature validation, preimage checks, delegation caveat enforcement).
     *
     * @param credential     the parsed credential to validate
     * @param requestContext per-request context (e.g., path, method, client_ip for L402
     *                       delegation caveats). Protocols that do not need request context
     *                       may ignore this parameter. Must not be {@code null} — pass an
     *                       empty map if no context is available.
     ...
     *
     * <p>The default implementation returns an empty {@link Optional}, indicating
     * that no receipt is produced. Protocols that support receipts (e.g., for
     * proof-of-payment) should override this method.
     *
