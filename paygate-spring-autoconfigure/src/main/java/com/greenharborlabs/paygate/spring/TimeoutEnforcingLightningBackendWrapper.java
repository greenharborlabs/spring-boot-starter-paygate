...
/**
 * Decorates a {@link LightningBackend} to enforce a hard deadline on all operations using virtual
 * threads. This is a defense-in-depth safety net: even if a backend implementation has a bug in its
 * own timeout handling, this wrapper guarantees calls will not block beyond the configured
 * duration.
 *
 * <p>Each operation is submitted to a virtual-thread-per-task executor and awaited with {@link
 * Future#get(long, TimeUnit)}. On timeout the virtual thread is interrupted and a {@link
 * LightningTimeoutException} is thrown.
 */
public class TimeoutEnforcingLightningBackendWrapper
    implements LightningBackend, java.io.Closeable {
