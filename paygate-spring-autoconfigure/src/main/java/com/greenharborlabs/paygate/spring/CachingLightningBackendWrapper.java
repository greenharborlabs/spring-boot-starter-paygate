...
/**
 * Decorates a {@link LightningBackend} to cache the result of {@link #isHealthy()} for a
 * configurable TTL. All other methods delegate directly to the wrapped backend.
 *
 * <p>Thread safety is achieved via a single {@code volatile} snapshot record; the slight risk of
...
public class CachingLightningBackendWrapper implements LightningBackend {
  ...
  /**
   * Returns the last cached health value without triggering a refresh call to the delegate. Returns
   * {@code false} if no health check has been performed yet.
   *
