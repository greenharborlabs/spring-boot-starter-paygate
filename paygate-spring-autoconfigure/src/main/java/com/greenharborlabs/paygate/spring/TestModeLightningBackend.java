 ...
 *
 * <p>Creates invoices with valid preimage/paymentHash pairs (where {@code paymentHash =
 * SHA-256(preimage)}) and fake bolt11 strings. The preimage is included on the returned {@link
 * Invoice} so the caller can expose it in test-mode responses, enabling end-to-end testing with
 * curl. {@link #lookupInvoice(byte[])} always returns {@link InvoiceStatus#SETTLED} with the
 * correct preimage. {@link #isHealthy()} always returns {@code true}.
 *
...
public final class TestModeLightningBackend implements LightningBackend {
  ...
  private static final System.Logger log =
      System.getLogger(TestModeLightningBackend.class.getName());
