 ...
 *
 * <p>Each key (typically a client IP) gets an independent token bucket stored in a Caffeine cache.
 * Caffeine handles bounded size (LRU eviction when full) and automatic expiry of idle entries,
 * eliminating manual cleanup logic.
 *
 * <p>Thread safety is achieved via a single {@code cache.asMap().compute()} call per {@code
 * tryAcquire}, which atomically creates or updates the bucket.
 *
 * <p>An injectable time source ({@link LongSupplier} for nanoTime) controls bucket refill
 * timestamps, enabling deterministic unit tests. A separate Caffeine {@link Ticker} controls cache
 * expiry timing.
 */
public class CaffeineTokenBucketRateLimiter implements PaygateRateLimiter {

  private static final System.Logger log =
      System.getLogger(CaffeineTokenBucketRateLimiter.class.getName());
   ...
   */
  public CaffeineTokenBucketRateLimiter(
      int maxTokens,
    ...
    this.cache =
        Caffeine.newBuilder()
            ...
            .ticker(ticker)
            .removalListener(
                (String key, Bucket value, RemovalCause cause) -> {
  ...
  /**
   * Returns the estimated number of active buckets in the cache. Triggers Caffeine maintenance
   * (cleanup of expired entries) first.
   */
