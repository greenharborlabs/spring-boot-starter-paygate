 ...
 *
 * <p>Used to remember which capability a token was issued for, so that subsequent requests carrying
 * the same token can be checked against the endpoint's required capability.
 */
public interface CapabilityCache {
   ...
   *
   * <p>If {@code capability} is {@code null} or empty, this method is a no-op (endpoints without
   * capabilities should not pollute the cache).
   *
