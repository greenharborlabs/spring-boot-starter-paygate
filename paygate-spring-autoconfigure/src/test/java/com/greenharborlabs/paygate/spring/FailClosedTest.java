 ...
 *
 * <p>Verifies that the filter returns 503 LIGHTNING_UNAVAILABLE when the Lightning backend is
 * unhealthy or when invoice creation fails. This is a critical security property: the system must
 * never grant access when the payment infrastructure is unavailable.
 *
 * <p>Written TDD-first per T066. The createInvoice-throws scenario is expected to fail until T072
 * adds try-catch handling in the filter.
 */
...
class FailClosedTest {
  ...
  class HealthCheckFails {
    ...
    @Test
    @DisplayName(
        "returns 400 when Authorization header has malformed L402 value even when Lightning is down")
    void returns400WithMalformedAuthHeaderWhenUnhealthy() throws Exception {
      mockMvc
          .perform(get(PROTECTED_PATH).header("Authorization", "L402 some-macaroon:some-preimage"))
  ...
  class CreateInvoiceThrows {
    ...
    @Test
    @DisplayName(
        "returns 503 with LIGHTNING_UNAVAILABLE when createInvoice fails (TDD: expected to fail until T072)")
