...
/**
 * Tests for {@link PaygateMetricsAutoConfiguration}, verifying that the
 * {@link PaygateMeterFilter} bean is created and correctly caps endpoint
 * tag cardinality based on {@link PaygateProperties} values.
 */
...
class PaygateMetricsAutoConfigurationTest {
    ...
    @Test
    @DisplayName("PaygateMeterFilter bean is created when paygate.enabled=true and Micrometer is on classpath")
    ...
    void filterCapsEndpointCardinality() {
        contextRunner.run(context -> {
            ...
            var endpointValues = registry.find("paygate.requests").counters().stream()
    ...
    void userProvidedFilterTakesPrecedence() {
        contextRunner
                .withBean("paygateMeterFilter", PaygateMeterFilter.class, () -> new PaygateMeterFilter(50, "_custom"))
                .run(context -> {
                    assertThat(context).hasSingleBean(PaygateMeterFilter.class);
