...
/**
 * Unit tests for {@link PaygateSecurityFilter} dual-protocol behavior.
 * Uses mock {@link PaymentProtocol} instances rather than real L402/MPP implementations.
 */
...
class DualProtocolFilterTest {
    ...
    @Mock
    private PaymentProtocol l402Protocol;
    ...
    void setUp() {
        ...
        endpointConfig = new PaygateEndpointConfig(
                "GET", "/api/protected", 100L, 3600L, "Test endpoint", "", "");
    ...
    private PaygateSecurityFilter createFilter(List<PaymentProtocol> protocols) {
        return new PaygateSecurityFilter(
                registry, protocols, challengeService, SERVICE_NAME,
                null, null, null, null);
    }

    private ChallengeContext createChallengeContext() {
        return new ChallengeContext(
                PAYMENT_HASH, TOKEN_ID, BOLT11, 100L,
                "Test endpoint", SERVICE_NAME, 3600L, "", ROOT_KEY, null, null);
    }

    // --- Test 1: Dual WWW-Authenticate headers on 402 ---

    @Test
    @DisplayName("Both protocols enabled, no auth header -> response has two WWW-Authenticate headers")
    void dualWwwAuthenticateHeadersOn402() throws Exception {
        ...
        when(l402Protocol.formatChallenge(any())).thenReturn(
                new ChallengeResponse("L402 macaroon=\"abc\", invoice=\"lnbc\"", "L402",
                        Map.of("macaroon", "abc")));
        when(mppProtocol.formatChallenge(any())).thenReturn(
    ...
    void l402AuthAccepted() throws Exception {
        ...
        when(l402Protocol.parseCredential("L402 token:preimage")).thenReturn(
                new PaymentCredential(PAYMENT_HASH, PREIMAGE, TOKEN_ID, "L402", null,
                        mock(ProtocolMetadata.class)));
    ...
    void mppAuthAccepted() throws Exception {
        ...
        when(mppProtocol.parseCredential("Payment method=\"lightning\"")).thenReturn(
                new PaymentCredential(PAYMENT_HASH, PREIMAGE, TOKEN_ID, "Payment", null,
                        mock(ProtocolMetadata.class)));
    ...
    void paymentReceiptOnMppSuccess() throws Exception {
        ...
        var receipt = new PaymentReceipt(
                "success", "challenge-id-1", "lightning", "ref-123",
    ...
    void cacheControlNoStoreOn402() throws Exception {
        ...
        when(mppProtocol.formatChallenge(any())).thenReturn(
                new ChallengeResponse("Payment challenge", "Payment", Map.of("token", "xyz")));
    ...
    void cacheControlPrivateOnReceipt() throws Exception {
        ...
        when(mppProtocol.parseCredential("Payment token")).thenReturn(
                new PaymentCredential(PAYMENT_HASH, PREIMAGE, TOKEN_ID, "Payment", null,
                        mock(ProtocolMetadata.class)));
        doNothing().when(mppProtocol).validate(any(), anyMap());
    ...
    void methodUnsupported400() throws Exception {
        ...
        when(mppProtocol.parseCredential("Payment bad-method")).thenReturn(
                new PaymentCredential(PAYMENT_HASH, PREIMAGE, TOKEN_ID, "Payment", null,
                        mock(ProtocolMetadata.class)));
        doThrow(new PaymentValidationException(
                ...
                .when(mppProtocol).validate(any(), anyMap());
    ...
    void unexpectedError503() throws Exception {
        ...
        when(mppProtocol.parseCredential("Payment crash")).thenReturn(
                new PaymentCredential(PAYMENT_HASH, PREIMAGE, TOKEN_ID, "Payment", null,
                        mock(ProtocolMetadata.class)));
    ...
    void singleSharedInvoice() throws Exception {
        ...
        when(l402Protocol.formatChallenge(l402ContextCaptor.capture())).thenReturn(
                new ChallengeResponse("L402 challenge", "L402", Map.of("macaroon", "abc")));
        when(mppProtocol.formatChallenge(mppContextCaptor.capture())).thenReturn(
                new ChallengeResponse("Payment challenge", "Payment", Map.of("token", "xyz")));
