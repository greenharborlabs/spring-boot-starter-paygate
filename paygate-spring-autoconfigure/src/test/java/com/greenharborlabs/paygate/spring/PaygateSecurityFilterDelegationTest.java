...
/**
 * Tests verifying that {@link PaygateSecurityFilter} populates
 * request metadata ({@code request.path}, {@code request.method}, {@code request.client_ip})
 * in the request context map passed to {@link PaymentProtocol#validate}.
 */
...
class PaygateSecurityFilterDelegationTest {
    ...
    /**
     * A structurally valid L402 Authorization header. The actual content does not matter
     * because the protocol mock accepts anything.
     */
    private static final String VALID_AUTH_HEADER = "L402 " + "AAAA".repeat(20) + ":" + "ab".repeat(32);
    ...
    @Captor
    private ArgumentCaptor<Map<String, String>> requestContextCaptor;
    ...
    void setUp() {
        filter = new PaygateSecurityFilter(
                registry, List.of(protocol), challengeService, SERVICE_NAME,
                null, null, null, null);
    ...
    private static PaymentCredential createStubCredential() {
        return new PaymentCredential(
                new byte[32], new byte[32], "ab".repeat(32), "L402", null,
                new ProtocolMetadata() {});
    }

    /**
     * Configures the registry to recognize the given method+path as a protected endpoint
     * and the protocol to accept any auth header and validate successfully.
     */
