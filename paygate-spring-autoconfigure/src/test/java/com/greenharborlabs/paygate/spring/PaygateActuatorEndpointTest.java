 ...
 *
 * <p>Verifies the JSON contract defined in l402-http-protocol.md section 7: GET /actuator/paygate
 * must return all required fields including enabled, backend, backendHealthy, testMode,
 * serviceName, protectedEndpoints, credentials, and earnings.
 *
 * <p>The {@code PaygateActuatorEndpoint} class does not exist yet (T092). These tests are written
 * first per TDD and will not compile until the endpoint is implemented.
 */
@SpringBootTest(
...
class PaygateActuatorEndpointTest {
    ...
    PaygateEndpointRegistry paygateEndpointRegistry() {
      ...
      registry.register(
          new PaygateEndpointConfig(
              "POST", "/api/v1/analyze", 10, 3600, "Financial data enrichment", "", ""));
      return registry;
    }
  ...
  class TopLevelFields {
    ...
    void returnsOk() throws Exception {
      mockMvc.perform(get("/actuator/paygate")).andExpect(status().isOk());
    }

    @Test
    @DisplayName("returns enabled field")
    void returnsEnabled() throws Exception {
      mockMvc.perform(get("/actuator/paygate")).andExpect(jsonPath("$.enabled", is(true)));
    }

    @Test
    @DisplayName("returns backend field")
    void returnsBackend() throws Exception {
      mockMvc.perform(get("/actuator/paygate")).andExpect(jsonPath("$.backend", is("lnbits")));
    }

    @Test
    @DisplayName("returns backendHealthy field")
    void returnsBackendHealthy() throws Exception {
      mockMvc.perform(get("/actuator/paygate")).andExpect(jsonPath("$.backendHealthy", is(true)));
    }

    @Test
    @DisplayName("testMode field is not present in response")
    void testModeNotPresent() throws Exception {
      mockMvc.perform(get("/actuator/paygate")).andExpect(jsonPath("$.testMode").doesNotExist());
    }

    @Test
    @DisplayName("returns serviceName field")
    void returnsServiceName() throws Exception {
      mockMvc.perform(get("/actuator/paygate")).andExpect(jsonPath("$.serviceName", is("my-api")));
    }
  }
  ...
  class ProtectedEndpoints {
    ...
    void endpointHasDescription() throws Exception {
      mockMvc
          .perform(get("/actuator/paygate"))
          .andExpect(
              jsonPath("$.protectedEndpoints[0].description", is("Financial data enrichment")));
    }
  ...
  class Credentials {
    ...
    void containsActiveCount() throws Exception {
      mockMvc.perform(get("/actuator/paygate")).andExpect(jsonPath("$.credentials.active", is(42)));
    }
  ...
  class Earnings {
    ...
    void containsVolatilityNote() throws Exception {
      mockMvc
          .perform(get("/actuator/paygate"))
          .andExpect(
              jsonPath("$.earnings.note", is("In-memory only; resets on application restart")));
    }
  }
  ...
  static class StubRootKeyStore implements RootKeyStore {
    ...
    public GenerationResult generateRootKey() {
      return new GenerationResult(
          new com.greenharborlabs.paygate.api.crypto.SensitiveBytes(key.clone()), new byte[32]);
    }
  ...
  /**
   * Stub earnings tracker that returns fixed values. The real {@link PaygateEarningsTracker} class
   * does not exist yet (T093/T094).
   */
