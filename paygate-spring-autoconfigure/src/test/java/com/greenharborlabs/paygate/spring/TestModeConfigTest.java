...
/**
 * Verifies that test-mode configuration provides a {@link TestModeLightningBackend} when {@code
 * paygate.test-mode=true}.
 *
 * <p>Per R-014, TestModeLightningBackend.createInvoice() returns a dummy Invoice with a random
 * payment hash and fake bolt11 string. lookupInvoice() always returns an Invoice with {@link
 * InvoiceStatus#SETTLED}. Preimage verification is skipped.
 *
 * <p>This test is written TDD-first: it references {@link TestModeLightningBackend} and {@link
 * TestModeAutoConfiguration}, which do not exist yet. The test will fail to compile until those
 * classes are implemented (T081, T082).
 */
...
class TestModeConfigTest {
  ...
  @Test
  @DisplayName(
      "TestModeLightningBackend not created when paygate.test-mode=true but paygate.enabled is not set")
  ...
  @Test
  @DisplayName(
      "TestModeLightningBackend not created when paygate.test-mode=true and paygate.enabled=false")
  void testModeNotActiveWhenMasterSwitchExplicitlyDisabled() {
    new WebApplicationContextRunner()
        .withConfiguration(AutoConfigurations.of(TestModeAutoConfiguration.class))
        .withPropertyValues(
            "paygate.enabled=false", "paygate.test-mode=true", "spring.profiles.active=test")
        .run(
            context -> {
              assertThat(context).hasNotFailed();
              assertThat(context).doesNotHaveBean(TestModeLightningBackend.class);
