 ...
 *
 * <p>{@link PaygateAutoConfiguration} is guarded by
 * {@code @ConditionalOnProperty(name = "paygate.enabled", havingValue = "true", matchIfMissing = false)},
 * so the entire configuration class — and all beans it declares — must be absent when
 * {@code paygate.enabled=false} or the property is not set at all.
 */
