 ...
 *
 * <p>This configuration shows how to use Paygate's Spring Security components to protect endpoints
 * with payment-based authorization. Key concepts:
 *
 * <ul>
 *   <li>{@code hasRole("PAYMENT")} accepts credentials from any payment protocol (both L402 and
 *       MPP). This role is granted to all authenticated payment tokens regardless of which protocol
 *       was used.
 *   <li>{@code hasRole("L402")} restricts access to L402 credentials only. MPP credentials will be
 *       rejected with a 403 Forbidden because they are not granted the {@code ROLE_L402} authority.
 ...
 *       {@code @PreAuthorize} expressions using it will continue to work.
 * </ul>
 */
@Configuration
@EnableWebSecurity
...
public class SecurityConfig {
  ...
  /**
   * Exposes the {@link AuthenticationManager} as a bean so that Paygate's auto-configuration can
   * create the {@link PaygateAuthenticationFilter}. Spring Security does not register the {@code
   * AuthenticationManager} as a bean by default — it is only accessible via {@link
  ...
  public SecurityFilterChain securityFilterChain(
      ...
      ObjectProvider<PaygateAuthFailureRateLimitFilter> rateLimitFilterProvider)
      throws Exception {
    ...
    return http.authenticationProvider(paygateProvider)
        ...
        .authorizeHttpRequests(
            auth ->
                auth
                    ...
                    .requestMatchers("/api/v1/l402-only")
                    .hasRole("L402")
                    ...
                    .anyRequest()
                    .authenticated())
        .exceptionHandling(ex -> ex.authenticationEntryPoint(paygateEntryPoint))
        .sessionManagement(
