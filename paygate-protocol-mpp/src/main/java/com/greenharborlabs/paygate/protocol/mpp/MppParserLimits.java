 ...
 */
public record MppParserLimits(
    int maxDepth, int maxStringLength, int maxKeysPerObject, int maxInputLength) {
  ...
  public MppParserLimits {
    ...
    if (maxStringLength <= 0) {
      throw new IllegalArgumentException(
          "maxStringLength must be positive, got " + maxStringLength);
    }
    if (maxKeysPerObject <= 0) {
      throw new IllegalArgumentException(
          "maxKeysPerObject must be positive, got " + maxKeysPerObject);
    }
    if (maxInputLength <= 0) {
