 ...
 *
 * <p>The challenge ID is computed over a pipe-delimited 7-slot input string: {@code
 * realm|method|intent|request_b64url|expires_or_empty|digest_or_empty|opaque_b64url_or_empty}
 *
 * <p>Absent optional fields use empty string in their slot. The resulting HMAC is encoded as
...
public final class MppChallengeBinding {
  ...
  /**
   * Constant-time verifies a challenge ID against the expected HMAC. Returns true if valid, false
   * if tampered.
   *
  ...
  private static byte[] computeHmac(
    ...
    String input =
        new StringBuilder()
            ...
            .append(requestB64)
            .append(PIPE)
            .append(nullToEmpty(expires))
            .append(PIPE)
            .append(nullToEmpty(digest))
            .append(PIPE)
