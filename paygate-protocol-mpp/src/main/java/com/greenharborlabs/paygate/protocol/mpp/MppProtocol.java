 ...
 *
 * <p>Uses the {@code Payment} authentication scheme with HMAC-SHA256 challenge binding
 * for stateless server-side verification. All validation follows a security-critical
 * order: preimage hash check, then HMAC binding, then expiry.
 *
...
public final class MppProtocol implements PaymentProtocol {
    ...
    public ChallengeResponse formatChallenge(ChallengeContext context) {
        ...
        var methodDetails = new LightningChargeRequest.MethodDetails(
                context.bolt11Invoice(), paymentHashHex, "mainnet");
        var chargeRequest = new LightningChargeRequest(
                String.valueOf(context.priceSats()),
                "BTC",
                context.description(),
                methodDetails);
        ...
        // RFC 3339 expires
        String expires = DateTimeFormatter.ISO_INSTANT.format(
                Instant.now().plusSeconds(context.timeoutSeconds()));

        // Handle opaque
        String opaqueB64 = null;
        if (context.opaque() != null) {
            String opaqueJcs = JcsSerializer.serialize(
                    new LinkedHashMap<>(context.opaque()));
            opaqueB64 = Base64.getUrlEncoder().withoutPadding()
        ...
        String id = MppChallengeBinding.createId(
                ...
                expires, context.digest(), opaqueB64, challengeBindingSecret);
    ...
    public void validate(PaymentCredential credential, Map<String, String> requestContext)
        ...
        boolean hmacValid = MppChallengeBinding.verify(
                id, realm, method, intent, request,
                expires, digest, opaque, challengeBindingSecret);
        if (!hmacValid) {
            throw new PaymentValidationException(
                    ErrorCode.INVALID_CHALLENGE_BINDING,
    ...
    /**
     * Sanitizes a string for use in an HTTP header quoted-string value.
     * Rejects control characters and double-quotes to prevent header injection.
     */
    private static String sanitizeHeaderValue(String value) {
        ...
        for (int i = 0; i < value.length(); i++) {
            ...
            if (c <= 0x1F || c == 0x7F || c == '"') {
                throw new IllegalArgumentException(
                        "Header value contains illegal character at index " + i
