...
/**
 * Verifies that {@code paygate.security-mode=servlet} forces servlet filter registration and skips
 * Spring Security beans, even though Spring Security is on the classpath.
 */
...
class SecurityModeServletTest {
  ...
  void servletFilterRegistrationCreated() {
    contextRunner.run(
        context -> assertThat(context.containsBean("paygateSecurityFilterRegistration")).isTrue());
  }
  ...
  void authenticationEntryPointNotCreated() {
    contextRunner.run(
        context -> assertThat(context).doesNotHaveBean(PaygateAuthenticationEntryPoint.class));
  }
  ...
  void authenticationProviderNotCreated() {
    contextRunner.run(
        context -> assertThat(context).doesNotHaveBean(PaygateAuthenticationProvider.class));
  }
