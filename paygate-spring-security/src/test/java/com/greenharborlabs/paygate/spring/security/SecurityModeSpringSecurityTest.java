...
/**
 * Verifies that {@code paygate.security-mode=spring-security} (explicit) enables
 * Spring Security beans and disables servlet filter registration.
 */
...
class SecurityModeSpringSecurityTest {
    ...
    void servletFilterRegistrationNotCreated() {
        contextRunner.run(context ->
                assertThat(context.containsBean("paygateSecurityFilterRegistration")).isFalse());
    }
    ...
    void authenticationEntryPointExists() {
        contextRunner.run(context ->
                assertThat(context).hasSingleBean(PaygateAuthenticationEntryPoint.class));
    }
    ...
    void authenticationProviderExists() {
        contextRunner.run(context ->
                assertThat(context).hasSingleBean(PaygateAuthenticationProvider.class));
    }
