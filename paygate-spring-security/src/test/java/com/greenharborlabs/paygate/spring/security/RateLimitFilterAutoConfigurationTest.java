...
/**
 * Verifies that {@link PaygateAuthFailureRateLimitFilter} bean creation is conditional on the
 * presence of a {@link PaygateRateLimiter} bean.
 */
...
class RateLimitFilterAutoConfigurationTest {
   ...
   */
  private final WebApplicationContextRunner fullContextRunner =
  ...
  /**
   * Minimal context runner with only PaygateSecurityAutoConfiguration and manually provided beans.
   * No PaygateRateLimiter is registered.
   */
  private final WebApplicationContextRunner minimalContextRunner =
      new WebApplicationContextRunner()
          .withConfiguration(AutoConfigurations.of(PaygateSecurityAutoConfiguration.class))
  ...
  @Test
  @DisplayName(
      "PaygateAuthFailureRateLimitFilter bean IS created when PaygateRateLimiter is present")
  void rateLimitFilterCreatedWhenRateLimiterPresent() {
    fullContextRunner.run(
        context -> assertThat(context).hasSingleBean(PaygateAuthFailureRateLimitFilter.class));
  }

  @Test
  @DisplayName(
      "PaygateAuthFailureRateLimitFilter bean is NOT created when PaygateRateLimiter is absent")
