 ...
 * protocol-agnostic (MPP) credential flows through the Spring Security layer.
 * Uses unit tests with mocks -- no Spring Boot context is loaded.
 */
...
class DualProtocolSecurityTest {
    ...
    class L402PopulatesSecurityContext {
        ...
        void l402CredentialFlowsThroughFilterAndProvider() throws Exception {
            L402Credential credential = createTestL402Credential(
                    List.of(new Caveat("service", "api")));
            ...
            var filter = new PaygateAuthenticationFilter(
                    authManager, List.of(mppProtocol), endpointRegistry);
    ...
    class DualHeader402EntryPoint {
        ...
        void entryPointIssuesMultipleWwwAuthenticateHeaders() throws Exception {
            ...
            var config = new PaygateEndpointConfig(
                    "GET", "/api/protected", 100, 3600, "Test", "", "read");
            when(endpointRegistry.findConfig("GET", "/api/protected")).thenReturn(config);

            var challengeContext = new ChallengeContext(
                    new byte[32], "aa".repeat(32), "lnbc1000n1test",
                    100, "Test", SERVICE_NAME, 3600, "", new byte[32], null, null);
            when(challengeService.createChallenge(any(), eq(config))).thenReturn(challengeContext);

            var entryPoint = new PaygateAuthenticationEntryPoint(
    ...
    class FilterRoutesProtocols {
        ...
        void l402HeaderProducesTokenWithComponentsNotAuthorizationHeader() throws Exception {
            ...
            verify(l402Validator).validate(componentsCaptor.capture(), any(L402VerificationContext.class));
            ...
            var token = (PaygateAuthenticationToken) SecurityContextHolder.getContext().getAuthentication();
    ...
    private PaymentCredential createTestPaymentCredential(String scheme, String source) {
        ...
        return new PaymentCredential(
                paymentHash, preimage, tokenId, scheme, source,
