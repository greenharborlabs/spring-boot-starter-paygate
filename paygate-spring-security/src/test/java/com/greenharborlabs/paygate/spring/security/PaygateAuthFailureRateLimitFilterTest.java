...
class PaygateAuthFailureRateLimitFilterTest {
    ...
    private static final String VALID_MACAROON = "AgELbXktc2VydmljZQJCAAA" +
    ...
    private static final String VALID_L402_HEADER =
            "L402 " + VALID_MACAROON + ":" + VALID_PREIMAGE;

    @Mock
    private PaygateRateLimiter rateLimiter;
    ...
    void preCheckRateLimitExhausted_returns429() throws ServletException, IOException {
        ...
        when(endpointRegistry.findConfig("GET", "/api/v1/data"))
                .thenReturn(new PaygateEndpointConfig("GET", "/api/v1/data", 10, 3600, "Data access", "", "read"));

        var filter = new PaygateAuthFailureRateLimitFilter(
    ...
    void nullRateLimiter_passthrough() throws ServletException, IOException {
        ...
        var filter = new PaygateAuthFailureRateLimitFilter(
                null, clientIpResolver, endpointRegistry, protocols);
        filter.doFilter(request, response, filterChain);
    ...
    void nullClientIpResolver_fallsBackToRemoteAddr() throws ServletException, IOException {
        ...
        var filter = new PaygateAuthFailureRateLimitFilter(
                rateLimiter, null, endpointRegistry, protocols);
        filter.doFilter(request, response, filterChain);
